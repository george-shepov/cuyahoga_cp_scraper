...
First marker: CR-23-684826-A (2023, case 684826)
Last marker: CR-25-707148-A (2025, case 707148)
"""
...
FIRST_CASE_NUM = 684826
LAST_CASE_YEAR = 2025
LAST_CASE_NUM = 707148

# Source and destination
SOURCE_DIR = Path("out")
DEST_DIR = Path.home() / "Desktop" / "CUYAHOGA_CASES"  # Default to Desktop
...
def main():
    ...
    print(f"📍 Range: CR-{FIRST_CASE_YEAR}-{FIRST_CASE_NUM:06d}-A to CR-{LAST_CASE_YEAR}-{LAST_CASE_NUM:06d}-A")
    print()
    ...
    summary_file.write_text(f"""CUYAHOGA COUNTY CASES EXTRACTION SUMMARY
========================================

Range: CR-{FIRST_CASE_YEAR}-{FIRST_CASE_NUM:06d}-A through CR-{LAST_CASE_YEAR}-{LAST_CASE_NUM:06d}-A
...
""")
    ...
    for year in sorted(cases_by_year.keys()):
        ...
        summary_file.write_text(
            f"  {year}: {count} cases (CR-{year}-{min_num:06d}-A to CR-{year}-{max_num:06d}-A)\n",
            encoding="utf-8"
        )
