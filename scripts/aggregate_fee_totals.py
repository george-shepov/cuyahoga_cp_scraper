...
with csv_in.open(newline='', encoding='utf-8') as f:
    ...
    for row in r:
        ...
        if amt is not None:
            ...
            # if no amount but 'SUBMITTED' treat as billed (no amount)
            if submitted_re.search(desc):
                count_billed[name] += 1

# write aggregate CSV
out_csv = root / 'other' / 'late_fee_aggregates.csv'
with out_csv.open('w', newline='', encoding='utf-8') as f:
    w = csv.writer(f)
    w.writerow(['name','billed_total','paid_total','count_billed','count_paid'])
    names = set(list(billed.keys()) + list(paid.keys()) + list(count_billed.keys()) + list(count_paid.keys()))
    for name in sorted(names, key=lambda n: (-billed.get(n,0), -paid.get(n,0), n)):
...
print('\nTop 10 by billed total:')
for i, (name, amt) in enumerate(sorted(billed.items(), key=lambda x: -x[1])[:10], 1):
...
print('\nTop 10 by paid total:')
for i, (name, amt) in enumerate(sorted(paid.items(), key=lambda x: -x[1])[:10], 1):
...
print(f"\nTotals — billed: ${total_billed:.2f}, paid: ${total_paid:.2f}")
