#!/usr/bin/env python3
"""
REVERSE SCRAPER - Start from END and work backward
- Starts at 707,148 and counts DOWN to 689,835 (where PARALLEL scraper stopped)
- Fills the gap in the middle of the range
- 8 parallel workers with throttling
...
def scrape_reverse():
    ...
    print(f"\n{'='*80}")
    print(f"REVERSE SCRAPER: {START} → {END} (BACKWARD)")
    print(f"{'='*80}")
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as executor:
        while cur >= END:
            ...
            for case in batch_cases:
                # Throttle requests
                now = time.time()
                wait_time = DELAY - (now - last_request_time)
                if wait_time > 0:
            ...
            pct = 100 * (start_pos - cur) / (start_pos - END)
